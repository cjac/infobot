...
sub process {
    ...
    if ($msgType =~ /private/ and $message =~ /^hey, what is/) {
        $infobots{$nuh} = $who;
        &msg($who,
	     "inter-infobot communication now requires ".
	     "version 0.43 or higher."
	     );
        return 'INTERBOT';
    }
    ...
    #    $confusedRE = join( '|',
    #			    map { quotemeta($_) } @confused
    #			  ) unless defined $confusedRE;
    ...
    # Half finished thought here - "^Pudge - it's there" looks like
    # math but is often nick completion or similar.
    # if (($message =~ s/^\S+\s*:\s+//) or
    #	 ($message =~ s/^\S+\s+--?\s+[.\d]//)
    #	) {
    ...
    # the thing to tell someone about ($tell_obj).  Yes i know these
    # are evil globals. --kl
